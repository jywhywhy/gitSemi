...
function list(page) {
	location.href="${path}/recipeBoard_servlet/list.do?search=${requestScope.search}&curPage="+page;
}
</script>
</head>
<body>
...
<div class="container list_a_div">
 <a href="${path}/recipeBoard_servlet/noticeList.do">공지</a>
</div>
<form id="form" name="form" method="post" action="${path}/recipeBoard_servlet/checkDelete.do">
    ...
    <c:forEach var="row" items="${list}">
     <c:choose>
      <c:when test="${row.show == 'Y' && row.notice == 0}">
       <tr>
        <c:if test="${sessionScope.admin == 'Y' || sessionScope.admin2 == 'Y'}">
         <td><input type="checkbox" class="check form-check-input" name="check" value="${row.num}"></td>
        </c:if>
